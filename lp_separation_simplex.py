...
# -------------------- PRIMAL (modelo del proyecto) --------------------
# Margen normalizado (=1) y holguras:
#   min (1/m)*1^T y + (1/p)*1^T z
#   s.a. Aw + y >= e_A*beta + e_A
#        Bw - z <= e_B*beta - e_B
#        y >= 0, z >= 0
# Para linprog usamos forma <=:
...
def build_primal(A: np.ndarray, B: np.ndarray):
    ...
    # c: sólo y y z aparecen en el objetivo
    N = n + 1 + m + p
    c = np.zeros(N)
    c[n+1:n+1+m] = 1.0 / m
    c[-p:] = 1.0 / p
...
# -------------------- DUAL explícito (para simplex) --------------------
# Derivado con “variable splitting” para que w y beta sean libres en el primal.
# Variables duales: λ_A in R^m, λ_B in R^p
#   0 ≤ λ_A ≤ 1/m,   0 ≤ λ_B ≤ 1/p
# Máx  -1^T λ_A - 1^T λ_B    <=>  Min  1^T λ_A + 1^T λ_B  (cambiando el signo)
# s.a.
#   -A^T λ_A +  B^T λ_B ≤ 0     (por w^+)
#    A^T λ_A -  B^T λ_B ≤ 0     (por w^-)
#    e_A^T λ_A -  e_B^T λ_B ≤ 0 (por beta^+)
#   -e_A^T λ_A +  e_B^T λ_B ≤ 0 (por beta^-)
...
def kkt_from_primal_dual(c: np.ndarray, A_ub: np.ndarray, b_ub: np.ndarray,
                         x_opt: np.ndarray, lam_opt: np.ndarray) -> dict:
    """
    KKT prácticos:
...
def run_simplex_experiment(outdir: str = "outputs_simplex") -> Dict[str, Any]:
    ...
    gap = abs(meta_p["obj"] - meta_d["obj_dual"]) if (meta_p["success"] and meta_d["success"]) else None
...
if __name__ == "__main__":
    ...
    print("\n=== SIMPLEX — Resumen ===")
    print(f"PRIMAL:  iters={out['primal']['iterations']}, cpu={out['primal']['cpu_seconds']:.6f}s, obj={out['primal']['objective']:.6f}, success={out['primal']['success']}")
    print(f"DUAL:    iters={out['dual']['iterations']}, cpu={out['dual']['cpu_seconds']:.6f}s, obj={out['dual']['objective']:.6f}, success={out['dual']['success']}")
    print(f"Gap |primal - dual| = {out['duality_gap_abs']}")
    print(f"KKT: min_slack={out['KKT']['min_slack']:.3e}, ||λ∘slack||_inf={out['KKT']['comp_inf']:.3e}, ||c + A^T λ||_inf={out['KKT']['station_inf']:.3e}")
